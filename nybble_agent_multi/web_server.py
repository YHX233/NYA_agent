...
class NybbleWebServer:
    ...
    def set_robot_manager(self, robot_manager) -> None:
        """Set the robot manager for multi-robot collaboration"""
    ...
    class Handler(http.server.SimpleHTTPRequestHandler):
        ...
        def do_POST(self) -> None:
            ...
            # Multi-robot POST endpoints
            elif parsed.path == '/api/robots/register':
                self._handle_register_robot(data)
            elif parsed.path == '/api/robots/unregister':
                self._handle_unregister_robot(data)
            elif parsed.path == '/api/groups/create':
                self._handle_create_group(data)
            elif parsed.path == '/api/groups/delete':
                self._handle_delete_group(data)
            elif parsed.path == '/api/groups/add-robot':
                self._handle_add_to_group(data)
            elif parsed.path == '/api/groups/remove-robot':
        ...
        # ==================== Multi-Robot API Handlers ====================
        
        def _handle_get_robots(self) -> None:
            """Get all registered robots"""
            if not self.web_server or not self.web_server.robot_manager:
                self._send_json({'robots': [], 'count': 0})
                return
            
            robots = self.web_server.robot_manager.get_all_robots()
            self._send_json({
                'robots': [r.to_dict() for r in robots],
                'count': len(robots),
                'my_robot_id': self.web_server.robot_manager.my_robot_id,
                'my_role': self.web_server.robot_manager.my_role.value
            })
        
        def _handle_get_robots_status(self) -> None:
            """Get status summary of all robots"""
            if not self.web_server or not self.web_server.robot_manager:
                self._send_json({'online': 0, 'offline': 0, 'busy': 0, 'total': 0})
                return
            
            robots = self.web_server.robot_manager.get_all_robots()
            online = len([r for r in robots if r.status.value == 'online'])
            offline = len([r for r in robots if r.status.value == 'offline'])
            busy = len([r for r in robots if r.status.value == 'busy'])
        ...
        def _handle_get_groups(self) -> None:
            ...
            if not self.web_server or not self.web_server.robot_manager:
                self._send_json({'groups': [], 'count': 0})
                return
            
            groups = self.web_server.robot_manager.get_all_groups()
            self._send_json({
                'groups': [g.to_dict() for g in groups],
                'count': len(groups)
            })
        
        def _handle_get_formations(self) -> None:
            """Get available formations"""
            formations = [
                {'id': 'none', 'name': 'None', 'description': 'No formation'},
                {'id': 'line', 'name': 'Line', 'description': 'Robots in a row'},
                {'id': 'column', 'name': 'Column', 'description': 'Robots in a column'},
                {'id': 'wedge', 'name': 'Wedge', 'description': 'V-shaped formation'},
                {'id': 'circle', 'name': 'Circle', 'description': 'Circular formation'}
            ]
            self._send_json({'formations': formations})
        
        def _handle_register_robot(self, data: Dict[str, Any]) -> None:
            """Register a new robot"""
            if not self.web_server or not self.web_server.robot_manager:
                self._send_error(500, 'Robot manager not initialized')
                return
            
            from robot_manager import RobotInfo, RobotRole, RobotStatus
            
            try:
                robot = RobotInfo(
                    id=data.get('id'),
                    name=data.get('name', f"Robot-{data.get('id')}"),
                    role=RobotRole(data.get('role', 'slave')),
                    status=RobotStatus(data.get('status', 'online')),
                    ip_address=data.get('ip_address', '127.0.0.1'),
                    port=data.get('port', 8080),
                    serial_port=data.get('serial_port', '/dev/serial0'),
                    capabilities=data.get('capabilities', [])
                )
                self.web_server.robot_manager.register_robot(robot)
                self._send_json({'success': True, 'robot': robot.to_dict()})
            except Exception as e:
                self._send_error(400, f'Invalid robot data: {str(e)}')
        
        def _handle_unregister_robot(self, data: Dict[str, Any]) -> None:
            """Unregister a robot"""
            if not self.web_server or not self.web_server.robot_manager:
                self._send_error(500, 'Robot manager not initialized')
                return
            ...
            success = self.web_server.robot_manager.unregister_robot(robot_id)
            self._send_json({'success': success})
        
        def _handle_create_group(self, data: Dict[str, Any]) -> None:
            ...
            group = self.web_server.robot_manager.create_group(name, robot_ids, formation)
            self._send_json({'success': True, 'group': group.to_dict()})
        
        def _handle_delete_group(self, data: Dict[str, Any]) -> None:
            ...
            success = self.web_server.robot_manager.delete_group(group_id)
            self._send_json({'success': success})
        
        def _handle_add_to_group(self, data: Dict[str, Any]) -> None:
            ...
            if not group_id or not robot_id:
                self._send_error(400, 'group_id and robot_id required')
                return
            
            success = self.web_server.robot_manager.add_to_group(group_id, robot_id)
            self._send_json({'success': success})
        
        def _handle_remove_from_group(self, data: Dict[str, Any]) -> None:
            ...
            success = self.web_server.robot_manager.remove_from_group(group_id, robot_id)
            self._send_json({'success': success})
        
        def _handle_sync_action(self, data: Dict[str, Any]) -> None:
        ...
        def _handle_set_formation(self, data: Dict[str, Any]) -> None:
            ...
            if not group_id or not formation:
                self._send_error(400, 'group_id and formation required')
                return
            
            success = self.web_server.robot_manager.set_formation(group_id, formation, spacing)
            
            # Calculate positions if successful
            positions = {}
            if success:
                positions = self.web_server.robot_manager.calculate_formation_positions(
                    group_id, 0, 0, 0
                )
